...
RTMPPublisher::RTMPPublisher(bool isBackup)
	:RTMPPublisher(){
...
DWORD WINAPI RTMPPublisher::CreateConnectionThread(RTMPPublisher *publisher)
{
	...
	String strBindIP;
	String strURL;
	if (publisher->isBackup){ 
		strURL = AppConfig->GetString(TEXT("Publish"), TEXT("URL_BACKUP"));
	}else{
		strURL = AppConfig->GetString(TEXT("Publish"), TEXT("URL"));
	}
